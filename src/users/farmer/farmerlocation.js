...
export const fetchLocalityDetails = async (client, locality, menus, id = null,) => {
    let results;
    if (locality === 'region') {
        ...
        let userCountySelection = countyIds[0].split(',')[`${
                (id -= 1)
            }`];
        ...
        let userSubcountySelection = subcountyIds[0].split(',')[`${
                (id -= 1)
            }`];
        ...
        let userLocationSelection = locationIds[0].split(',')[`${
                (id -= 1)
            }`];
...
export const changeUserLocation = async (textValue, text, client, menus) => {
    ...
    if (textValue === 3) {
        ...
        const postLocationDetails = await retreiveCachedItems(client, ['userCountySelection', 'userSubCountySelection', 'userLocationSelection', 'user_id',]);
